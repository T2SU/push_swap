...
/*   Updated: 2021/06/21 05:31:15 by smun             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
**      create a new line with the index we found, and return kStatusFoundLine.
**      However, if not found, it returns kStatusContinue
...
static int			read_line(t_reader *reader, char **line)
{
	...
	if (index >= 0)
	{
		...
		return (reader->eof ? kStatusEOF : kStatusFoundLine);
	}
...
int					get_next_line(int fd, char **line)
{
	...
	if (readers[fd] == NULL && !(readers[fd] = new_reader()))
		return (kReturnError);
	while ((status = read_line(readers[fd], line)) == kStatusContinue)
	{
	...
	if (status == kStatusFoundLine)
		return (kReturnFoundLine);
	if (readers[fd]->buffer)
		free(readers[fd]->buffer);
	free(readers[fd]);
	readers[fd] = NULL;
	return (len == 0 ? kReturnEOF : kReturnError);
}
